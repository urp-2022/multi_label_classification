...
for e in range(EPOCH):
    ...
    for i, (images, targets) in tqdm(enumerate(train_hard_loader), total=len(train_hard_loader)):
    ...
    total_train_loss = train_loss / len(train_hard_loader)
    scheduler.step()
    ...
    if best_loss > total_valid_loss:
        print("model saved\n")
        torch.save(model.state_dict(), 'model_origin_aug.h5')
